...
trait JsValueGenerators {
   ...
   *
   * @param includeUndefined includes [[JsUndefined]] unless set to false
   */
  def genJsPrimitive(includeUndefined: Boolean = true): Gen[JsValue] = {
    ...
    if (includeUndefined) {
      genPrims :+= Gen.const(JsUndefined("[generated by genJsPrimitive]"))
    }
  ...
  def genJsValue(implicit maxDepth: Depth = defaultMaxDepth, maxWidth: Width = defaultMaxWidth): Gen[JsValue] = {
    if (maxDepth === 0) genJsPrimitive(includeUndefined = false)
    else Gen.oneOf(
      genJsPrimitive(includeUndefined = false),
