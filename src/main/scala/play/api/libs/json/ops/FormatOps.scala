...
trait FormatAsEither {
   ...
   *
   * @note if you need more than two potential subclasses and you have some field to key on,
   *       you can use Json.formatAbstract()
   *
   ...
   */
  class EitherFormatBuilder[Left, Right] private[FormatAsEither]
    ...
    /**
     * Creates a play.libs.json.Format from two conditions, the first condition dictates how to deserialize the Json,
     * and the second format dictates how to serialize it. In both conditions, the "Left" or "Failed" format is false,
     * and the "Right" or "Success" format is true.
     *
